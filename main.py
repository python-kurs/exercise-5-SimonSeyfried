...
dat_dir = Path("C:/Users/seysi/Documents/Python Scripts/ex_5_data")

out_dir = Path("C:/Users/seysi/Documents/GitHub/exercise-5-SimonSeyfried/out")
...
dat = dat_dir / "tg_ens_mean_0.25deg_reg_v19.0e.nc"
ex = xr.open_dataset(dat)
ex_full = ex.sel(latitude = slice(30,72), longitude = slice(-13,25), time = slice("1981-01-01","2010-12-31"))
ex_slice_mean = ex_full.groupby("time.month").mean("time")
...
ex_2018 = ex.sel(latitude = slice(30,72), longitude = slice(-13,25), time = slice("2018","2018"))
ex_per_2018 = ex_2018.groupby("time.month").mean("time")
anom_2018 = ex_per_2018 - ex_slice_mean 
anom_2018["tg"].plot()
...
ex_mean_all = ex.sel(latitude = slice(30,72), longitude = slice(-13,25), time = slice("1981-01-01","2010-12-31")).mean("time")
ex_mean_2018 = ex.sel(latitude = slice(30,72), longitude = slice(-13,25), time = slice("2018","2018")).mean("time")
anom_all_2018 = ex_mean_2018 - ex_mean_all


# location and comparison

anom_mr = anom_all_2018.sel(latitude = 50.80, longitude = 8.77, method = "nearest")

if anom_mr > anom_all_2018:
